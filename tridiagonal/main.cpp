...
CLHRESULT TestSolvingDiagonalEquation(cl_command_queue cmd_queue, size_t dimx) {
  ...
  V_RETURN2(d_d <<=
            ...
            hr);
  V_RETURN2(x_d <<=
            clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_HOST_READ_ONLY,
                           buffer_len, nullptr, &hr),
            hr);
...
int main() {
  ...
  V_RETURN(CreateProgramFromILFile(context, device, "OCL-SpirV/tridiagonal.spv",
                                   &g_pTridiagProgram));
