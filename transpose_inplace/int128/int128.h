...
#ifndef _M_X64
#error Int128 is available only in x64 architecture
#endif
...
public:
  ...
  inline _int128 operator << (unsigned int bits) const {
    if(bits < 64) {
      unsigned int rembits = 64 - bits;
      unsigned __int64 over = ((-1ll << rembits) & lo) >> rembits;
      return _int128(lo << bits, over | (hi << bits));
    } else {
      return _int128(0, lo << (bits - 64));
    }
  }
  ...
  inline _int128 operator >> (unsigned int bits) const {
    if(bits < 64) {
      unsigned int rembits = 64 - bits;
      unsigned __int64 under = ((-1ull >> rembits) & hi) << rembits;
      return _int128(under & ((unsigned __int64)lo >> bits), (__int64)hi >> bits);
    } else
      return _int128(hi >> (bits - 64), (__int64)hi >> 63);
  }

  inline _int128& operator <<= (unsigned int bits) {
    *this = *this << bits;
    return *this;
  }
...
class _uint128 {
private:
  _uint128(unsigned __int64 _lo, const unsigned __int64 _hi) : lo(_lo), hi(_hi) {}
...
public:
  ...
  inline _uint128 operator<<(unsigned int bits) const {
    if (bits < 64) {
      unsigned int rembits = 64 - bits;
      unsigned __int64 over = ((-1ll << rembits) & lo) >> rembits;
      return _uint128(lo << bits, over | (hi << bits));
    } else {
      return _uint128(0, lo << (bits - 64));
    }
  }
  ...
  inline _uint128 operator>>(unsigned int bits) const {
    if (bits < 64) {
      unsigned int rembits = 64 - bits;
      unsigned __int64 under = ((-1ull >> rembits) & hi) << rembits;
      return _uint128(under & (lo >> bits), hi >> bits);
    } else
      return _uint128(hi >> (bits - 64), 0);
  }

  inline _uint128 &operator<<=(unsigned int bits) {
    *this = *this << bits;
    return *this;
  }
