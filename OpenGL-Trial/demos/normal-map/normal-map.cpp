...
int main() {
  ...
  // create main window
  window = glfwCreateWindow(windowWidth, windowHeight, "normal-map", nullptr, nullptr);
  if (!window) {
...
int create_shader_programs() {
  ...
  if(NormalMapProgram.Create("shaders/normal-map.vs", "shaders/normal-map.fs"))
    return -1;
...
int create_textures() {
  ...
  if(LoadTexture("../../../asset/textures/brickwall.jpg", &AlbedoTexture))
    return -1;
  ...
  if(LoadTexture("../../../asset/textures/brickwall_normal.jpg", &NormalTexture))
    return -1;
