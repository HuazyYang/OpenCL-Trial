...
class CubeRenderContext : public VulkanRenderContext {
 public:
  ...
  virtual void Update(float fTime, float fTimeElapsed) override {
    ...
    glm::mat4 matRotate = glm::rotate(glm::mat4(1.0f), fTimeElapsed * glm::two_pi<float>(), glm::vec3(.0f, 0.f, 1.0f));
    ...
    glm::mat4 matGLSLTexcoordsFixup(1.0f, .0f, .0f, .0f, .0f, -1.0f, .0f, .0f, .0f, .0f, 1.0f, .0f, .0f, 1.0f, .0f,
                                    1.0f);
  ...
  virtual void RenderFrame(float fTime, float fTimeElapsed) override {
    ...
    VkDescriptorSet descriptorSets[] = {m_aDescriptorSets[m_iCurrRendererItem], m_pDiffuseDiscriptorSet};

    vkCmdBindDescriptorSets(pCmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pPipelineLayout, 0, _countof(descriptorSets),
 ...
 private:
  VKHRESULT LoadTextures() {
    ...
    V_RETURN(m_aDiffuseMap.LoadFromDDSFile(m_pDevice, pCmdBuffer, L"Media/Textures/DX11/flare.dds",
                                           VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                                           VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT));

    V_RETURN(m_aMaskDiffuseMap.LoadFromDDSFile(m_pDevice, pCmdBuffer, L"Media/Textures/DX11/flarealpha.dds",
                                               VK_ACCESS_SHADER_READ_BIT, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                                               VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT));
...
#else
    shaderModules[0] = CreateShaderModuleFromSPIRVFile(m_pDevice, L"shaders/box.vs.spv");
    shaderModules[1] = CreateShaderModuleFromSPIRVFile(m_pDevice, L"shaders/box.fs.spv");
#endif
...
#else
    shaderStageInfos[0].pName = "VSMain";
#endif
...
#else
    shaderStageInfos[1].pName = "PSMain";
#endif
  ...
  VKHRESULT CreatePiplineLayout() {
    ...
    if (!m_pDiffuseDescriptorSetLayout) {
      VkDescriptorSetLayoutBinding layoutBindings[] = {
          ...
          {2, VK_DESCRIPTOR_TYPE_SAMPLER, 1, VK_SHADER_STAGE_FRAGMENT_BIT, &(m_aStaticSamplers[0])},
          {3, VK_DESCRIPTOR_TYPE_SAMPLER, 1, VK_SHADER_STAGE_FRAGMENT_BIT, &(m_aStaticSamplers[1])},
#endif
      };
...
VulkanRenderContext *CreateSampleRenderContext() {
  return new CubeRenderContext();
}
